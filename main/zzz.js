...
zzz.api.danmu = {
    ...
    create: function (danmu, video, container, relatedVideoElement) {
        ...
        try {
            ...
            var ct = player.containerEl,
                cv = ct.getElementsByTagName("canvas")[0];
            zzz.anim.set(ct, {
                left: 0,
                top: 0,
                width: "auto",
            ...
            player.hide = function (willHide) {
                var control = this.containerEl.getElementsByClassName("control-layer")[0],
                    video = this.videoEl;
...
zzz.api.render = {
    readCommand: function (text) {
        ...
        while (i !== -1) {
            ...
            if (index === -1) {
                if (cmd === "block") {} else if (cmd === "follow") {
                    command["follow"] = true;
                }
                //if simple class
                else {
            ...
            //normal case
                ...
                let attribute = cmd.slice(0, index),
                    value = cmd.slice(index + 1);
                if (attribute === "style") {
    ...
    splitCSSText: function (text) {
        ...
        for (let j of text.split(";")) {
            let k = j.indexOf(":"),
                key = j.slice(0, k).trim(),
                val = j.slice(k + 1).trim();
    ...
    readText: function (text) {
        ...
        while (max_times--) {
            ...
            if (i !== text.length && i < j) {
                ...
                let key = cmd.slice(0, index),
                    val = cmd.slice(index + 1);
                //end
                if (key === "/") {
                    if (tags) {
                ...
                //font size
                    ...
                    tags.push(this.createTag({
                        tag: "font",
                        style: {
                            fontSize: val + val.search(/[empxn]/) === -1 ? "em" : ""
                        }
                    }));
                }
                //hyperlink,a tag
                ...
                //footnote
                    ...
                    tags.push(this.createTag({
                        tag: "a",
                        attribute: {
                            href: "#" + footnoteindex[1],
                            class: "footnoteinnerindex"
                        }
                    }));
                } else if (key && val) {
                    let attr = {};
    ...
    essay: function (essay) {
        try {
            ...
            if (essay.title) zzz.create("div", {
                innerHTML: titleText,
                className: "title"
            }, null, zzz.get.id("content"));
            if (essay.author) zzz.create("div", {
                innerHTML: this.text(essay.author),
                className: "author"
            }, null, zzz.get.id("content"));
    ...
    createFootnote: function (uid) {
        return {
            ...
            note: [],
            rd: function (t) {
                return zzz.api.render.text(t, true)
            },
            link: function (index) {
...
zzz.api.steam = {
    ...
    localurl:"http://127.0.0.1:27060/steam",//specefic port allowed.
    queue_index: 0,
    ...
    generateData: function (cdk) {
        return "product_key=" + cdk + "&sessionid=" + this.id;
    },
    send: function (c) {
        var short = zzz.api.steam;
        fetch(this.url, {
            body: short.generateData(c),
            method: "POST",
            headers: {
                'Content-type': "application/x-www-form-urlencoded;charset=UTF-8"
            }
    ...
    sync_send: function (cdk) {
        var response = zzz.fetch.ajax({
            url: this.url,
            method: "post",
            data: this.generateData(cdk),
            async: false,
            form: 1
        }).responseText;
        try {
        ...
        return result.purchase_result_details || this.error;
    },
    ...
    cope: function (str) {
        ...
        for (let j = 0; j <= 6; j++)
            setTimeout(function () {
                zzz.api.steam.register(str.replace("?", j.toString()));
            }, j * 1000);
    },
    run: function () {
    ...
    local: function (url) {
        if (!url) {
            zzz.fetch.get(this.localurl, zzz.api.steam.local);
        } else {
            try {
                var response = zzz.fetch.ajax({
                    url: this.localurl,
    ...
    batch_code: function (arr) {
        var count = 0,
            success_count = 0;
        for (var i of arr) {
            for (var cdk of i) {
                if (this.history.include(cdk)) continue;
                var result = this.sync_send(cdk);
                zzz.time.sleep(1000);
                count++;
                this.history.push(cdk);
                if (result === this.full) {
                    console.log("mark:", success_count, "/", count);
                    return;
                } else if (result === this.error) {
                    console.log("error happened");
                    return;
                } else if (result === this.success) {
                    console.log("cdk", cdk, "register succeeded!");
                    success_count++;
                    break;
                }
            }
...
zzz.api.tieba = {
    ...
    replyFloor: function (floor, text) {
        ...
        var replyStatusElement = short[i][1].getElementsByClassName("j_lzl_r p_reply")[0].firstElementChild,
            replyStatus;
...
zzz.api.translation = {
    getURL: function (engine, text, fromLanguage, toLanguage, id, token) {
        ...
        if (engine === "baidu")
            result = result.replace("{sign}", zzz.code.md5.encode(id + text + salt.toString() + token));
            ...
            result = result.replace("{sign}", zzz.code.sha256.encode(id + input + salt + time.toString() + token.toString()))
        }
        return result;
    },
...
zzz.api.archive = {
    save: function (url, callback) {
        return zzz.fetch.create('https://web.archive.org/save/' + zzz.path.deleteEnd(url), {
    ...
    open: function (url, func) {
        ...
        zzz.fetch.get('http://archive.org/wayback/available?url=' + zzz.path.deleteEnd(url), {
            callback: callback
        });
    },
    find: function (url) {
...
//OCR API
//source:baidu
//TODO : need recheck after reconstruction of fetch
zzz.api.ocr = {
...
zzz.api.danmu = {
    init: function () {
        ...
        zzz.fetch.js(src + js_src);
        zzz.fetch.css(src + css_src);
        var danmu_url=prompt("请输入弹幕地址",zzz.storage.get("danmu")||"");
        if(danmu_url.indexOf("http")===-1&&danmu_url.indexOf("file://")===-1) danmu_url="file:///"+danmu_url;
        var video_url=prompt("请输入视频地址",zzz.storage.get("video")||"");
        zzz.storage.set("danmu",danmu_url);
        zzz.storage.set("video",video_url);
        window.player=zzz.api.danmu.create(danmu_url,video_url,null);
        players.resize();
    },
    players: [],
    id: 0,
    create: function (danmu, video, container, relatedVideoElement) {
        ...
        try {
            ...
            this.players[this.players.length] = player;
            var ct = player.containerEl,
                cv = ct.getElementsByTagName("canvas")[0],vdo=ct.getElementsByTagName("video")[0];
            zzz.anim.set(ct, {
                left: 0,
                top: 0,
                width: "auto",
