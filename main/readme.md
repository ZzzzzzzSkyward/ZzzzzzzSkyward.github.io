...
2.2添加跳转方法：只输入add一个单词，在第一次跳出的框里填写名称，在第二次跳出的框里填写网址，并把结果复制粘贴到settings.js里面的相应位置。以后会添加自动保存方法。但是由于浏览器本身策略，无法从网页直接保存到本地。  
2.2.1  
...
10webarchive API（没有UI界面，需要调用zzz.api.archive.save(url)方法和zzz.api.archive.open(url)方法 )   
